...
- **0.9.25–0.9.29:** no exact release messages have been recovered yet. They are intentionally absent rather than guessed.
- **Any unlisted 0.13.x / 0.15.x numbers:** current evidence shows jumps (for example 0.13.3 → 0.14.0 and 0.15.7 → 0.16.0); no files are created unless a release is actually found.
- If another historical chat reveals a reused version number, the additional state belongs under `variants/` instead of overwriting the canonical note.

## Reused version numbers

- **0.17.12** has a canonical functional release note plus a separate public-repository-preparation variant under `variants/`.
